...
'''[数据库表结构]
[shanghai_lianjia_short数据库表结构]
CREATE TABLE `shanghai_lianjia_short` (
...
OutFile  = open("shanghai_lianjia_short_1_"+DateTime+".txt","a",encoding='utf-8')
OutFile1 = open("shanghai_lianjia_short_2_"+DateTime+".txt","a",encoding='utf-8')
...
def connLinajia():
    global conn,sql
    conn = pymysql.connect(host='localhost', port=3306, user='root', passwd='root', db='lianjia', charset='utf8')
    cur = conn.cursor()
...
def insertLianjia(fileName,flag):
    ...
    while True:
        ...
        if int(flag) == 1:
            ...
            sql="INSERT IGNORE INTO `shanghai_lianjia_short` (`area`,`area_en`,`fangxing`,`fangxing_en`,`allhouse`,`createdate`) VALUES (%s,%s,%s,%s,%s,%s)"
            try:
            ...
            connLinajia()
            DateTime =time.strftime("%Y-%m-%d %H:%M:%S", time.localtime());
            sql="INSERT IGNORE INTO `shanghai_lianjia_short_info` (`title`,`xiaoqu`,`fangxing`,`mianji`,`chaoxiang`,`zhuangxiu`,`floor`,`guanzhu`,`daikan`,`fabu`,`totalPrice`,`unitPrice`,`area`,`areaEN`,`fx`,`fxEN`) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)"
            try:
                with conn.cursor() as cursor:
                    #print(title+"\t"+xiaoqu+"\t"+fangxing+"\t"+mianji+"\t"+chaoxiang+"\t"+zhuangxiu+"\t"+floor+"\t"+guanzhu+"\t"+daikan+"\t"+fabu+"\t"+totalPrice+"\t"+unitPrice)
...
def makepie():
    ...
    sql1 ='select area,sum(allhouse)as alls,date_format(createdate, "%Y-%m-%d")  from shanghai_lianjia_short  GROUP BY area,date_format(createdate, "%Y-%m-%d") order by sum(allhouse) desc '
    cursor.execute(sql1)
...
if __name__ == '__main__':
    ...
    #time.sleep(60)
    #lianjiaA2()
    #truncate(tableName='shanghai_lianjia_short')
    #truncate(tableName='shanghai_lianjia_short_info')
    #insertLianjia("shanghai_lianjia_short_1_"+DateTime+".txt",flag=1)
    #insertLianjia("shanghai_lianjia_short_2_"+DateTime+".txt",flag=2)
    #makepie()
    lianjia2()